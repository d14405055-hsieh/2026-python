...
把每個數字視為七段顯示器 bitmask，枚舉「移動一根木棒」的兩種情況：

1. 在同一個數字內搬移（段數不變，bit 差異為 2）。
2. 從某數字移除 1 段，補到另一數字增加 1 段。

每次修改後，因 `+/-/=` 都不變，只要重新檢查等式左右值是否相等。找到第一個可行解就輸出（加 `#`），否則輸出 `No`。

## 解題代碼

```python
# AI 教你的簡單版本與手打版本，請見：
# weeks/week-07/solutions/1114405055/solve_10101-easy.py
# weeks/week-07/solutions/1114405055/solve_10101.py
```
...
```python
# 測試程式
# weeks/week-07/solutions/1114405055/test_10101.py
```

```text
# 你手打程式的測試 LOG
# weeks/week-07/solutions/1114405055/test_10101.log
# 內容摘要：2 passed in 0.06s
```
